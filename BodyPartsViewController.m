...
@interface BodyPartsViewController ()

@property (weak, nonatomic) IBOutlet UIView *view_TF;
@property (weak, nonatomic) IBOutlet UITextField *textField_body;
...
- (void)viewDidLoad {
    ...
    self.textField_body.backgroundColor = [UIColor clearColor];
    self.textField_body.borderStyle = UITextBorderStyleNone;
    self.textField_body.frame = CGRectMake(10, 5, VIEW_SIZE.width-20, 40);
...
{
    NSArray * arr = @[@"小蛮腰",@"性感双唇",@"美胸",@"美臀",@"芊芊玉手",@"大眼睛",@"美腿",@"锁骨",@"眼睛",@"赚钱男",@"高鼻梁",@"八块腹肌",@"黝黑的皮肤",@"富有财气大鼻子",@"络腮胡",@"酒窝",@"胸毛"];
...
-(void)tapProfessionBtn:(UIButton *)sender
{
    if (sender.selected == NO) {
        ...
        _textField_body.text = [_textField_body.text stringByAppendingString:[NSString stringWithFormat:@" %@;",str]];
        
    }else{
        sender.selected = NO;
        sender.backgroundColor = [UIColor whiteColor];
        // 删除
        _textField_body.text = [_textField_body.text stringByReplacingOccurrencesOfString:[NSString stringWithFormat:@" %@;",sender.titleLabel.text] withString:@""];
    }
}
